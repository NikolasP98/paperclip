...
function buildAgentSnippet(input: AgentSnippetInput) {
  ...
  const connectivityBlock =
    candidateUrls.length === 0
      ? `No candidate URLs are available. Ask your user to configure a reachable hostname in MINION, then retry.
Suggested steps:
- choose a hostname that resolves to the MINION host from your runtime
- run: pnpm paperclipai allowed-hostname <host>
- restart MINION
- verify with: curl -fsS http://<host>:3100/api/health
- regenerate this invite snippet`
      : `If none are reachable, ask your user to add a reachable hostname in MINION, restart, and retry.
Suggested command:
- pnpm paperclipai allowed-hostname <host>
Then verify with: curl -fsS <base-url>/api/health`;

  const resolutionLine = resolutionTestUrl
    ? `\nYou MUST test MINION-to-gateway reachability, call: ${resolutionTestUrl}?url=<urlencoded-gateway-url> (using the hostname that worked above). Do not assume your 172.x is necessarily reachable from MINION. Test it. `
    : "";

  return `You're invited to join a MINION organization as a Minion gateway agent.

The URLs you should try are:
${candidateList}

Connectivity guidance:
If you are running on a different machine than MINION, MINION must be reachable at one of the hostnames used above.
Verify the hostname works from your runtime with: GET <base-url>/api/health

${connectivityBlock}

For your "agentName", just use your own agent name and send it directly.
You also need to send a gateway URL that MINION can access. Check your config and include that URL.${resolutionLine}
...
- Keep device auth enabled (recommended). If \`devicePrivateKeyPem\` is omitted, MINION will generate and persist one during join so pairing approvals remain stable.
...
Then after you've connected to MINION (exchanged keys etc.) you MUST review and follow the onboarding instructions in onboarding.txt they give you.
