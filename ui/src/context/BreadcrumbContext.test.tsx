...
describe("BreadcrumbContext", () => {
  ...
  it("builds page titles with the selected company name before MINION", () => {
    expect(buildDocumentTitle([{ label: "Inbox" }], "Anachronist Wiki")).toBe(
      "Inbox • Anachronist Wiki • MINION",
    );
    expect(
      buildDocumentTitle(
    ...
    ).toBe("PAP-3515 • Issues • Anachronist Wiki • MINION");
  });

  it("omits blank company names from page titles", () => {
    expect(buildDocumentTitle([{ label: "Inbox" }], "  ")).toBe("Inbox • MINION");
    expect(buildDocumentTitle([], null)).toBe("MINION");
  });
});
