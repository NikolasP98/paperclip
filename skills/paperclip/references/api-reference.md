...
## Minion Invite Prompt (CEO)

Use this endpoint to generate a short-lived Minion onboarding invite prompt. (The legacy `openclaw` segment in the route path is preserved for API stability.)

```
POST /api/companies/{companyId}/openclaw/invite-prompt
{
  "agentMessage": "optional note for the joining Minion agent"
}
```
...
| POST   | `/api/companies/:companyId/openclaw/invite-prompt` | Generate Minion invite prompt (CEO/board only) |
